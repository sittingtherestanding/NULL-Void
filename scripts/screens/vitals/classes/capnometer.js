var Capnometer = function()
{
	...
	this.draw = function(x, y, width, height)
	{
		...
		if (this.history.length >= width)
		{
			...
			{
				this.pencil.setStroke(2).setColor(colorOne).setPosition(x + this.currentX - 1, y + height / 2 + this.history[i - 1]).setEndPosition(x + this.currentX, y + height / 2 + this.history[i]).strokeLine()
